...
# Оптимизированная функция форматирования с компиляцией regex и escaping HTML entities в содержимом
...
def bold_match(m):
    return f'<b>{escape_html(m.group(2))}</b>'

def italic_match(m):
    return f'<i>{escape_html(m.group(2))}</i>'

def code_match(m):
    return f'<code>{escape_html(m.group(1))}</code>'

def pre_match(m):
    return f'<pre>{escape_html(m.group(1))}</pre>'

def format_to_html(text: str) -> str:
    text = bold_re.sub(bold_match, text)
    text = italic_re.sub(italic_match, text)
    text = code_re.sub(code_match, text)
    text = pre_re.sub(pre_match, text)
    return text
